...
require '/usr/share/nginx/phplibs/tcpdf/tcpdf.php';

class ProtocolController
{
    public function topdfAction($oderId=null)
    {
        
    }
    
    public function viewAction($oderId=null)
    {
    ...
    protected function fillhtml($arrOrder,$tpl)
    {
        
    }
    protected function toPDF($html)
    {
        ...
        /*画一条线：
        x1：线条起点x坐标        y1：线条起点y坐标
        x2：线条终点x坐标        y2：线条终点y坐标
        ...
        */
        ...
        W：设置多行单元格的宽        H： 设置多行单元格的单行的高
        Text：文本
        Border：边框
        Align：文本位置
        Fill：填充
        Ln：0，单元格后的内容插到表格右边或左边，1，单元格的下一行，2，在单元格下面
        X：设置多行单元格的行坐标        Y：设置多行单元格的纵坐标
        ...
        Align：调整文本位置。 */
        $pdf->writeHTML(file_get_contents(__DIR__.'/pdf.html'));
	//$pdf->writeHTML('<div>中文</div><div>test</div>');
