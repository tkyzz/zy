...
class ActBirth extends \Prj\Bll\_BllBase
{
    ...
    public function listenBirth($params = []){
        ...
        $bank->load();
        \Prj\Loger::out(\Prj\Model\UserBank::db()->lastCmd());
        if(!$bank->exists())return $this->resultError('银行卡信息不存在!!!');
        try{
            $idNum = $bank->getField('idNumb');
        }catch (\Exception $e){
            $idNum = '';
        }
        ...
        if(strlen($idNum)==18)
        {
            $tyear=intval(substr($idNum,6,4));
            $tmonth=intval(substr($idNum,10,2));
            $tday=intval(substr($idNum,12,2));
        }
        elseif(strlen($idNum)==15)
        {
            $tyear=intval("19".substr($idNum,6,2));
            $tmonth=intval(substr($idNum,8,2));
            $tday=intval(substr($idNum,10,2));
        }
        if($tmonth != date('m')){
    ...
    protected function getBirthUserListByMonth($params = []){
        ...
        $records = \Prj\Model\JzUserFinal::getRecords(null , $where , 'sort ymdReg sort hisReg' , $pageSize , $pageNo * $pageSize);
        \Prj\Loger::outVal('sql' , \Prj\Model\JzUserFinal::db()->lastCmd());
