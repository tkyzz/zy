...
{
    public function crond($date){
        \Prj\Loger::$prefix = '[Crond-getChannelStatics]';
        $hour = date('H');
        if($hour == 23){
            $this->getChannelStaticsInfo($date);
        }

    }

    protected function getChannelStaticsInfo($date){
        Loger::$prefix = "[getChannelStatics]";
        $today = $date?date("Ymd",strtotime($date)):date("Ymd");
        Loger::outVal("today",$today);
        $contractTbName= \Prj\Model\ContractInfo::getTbname();
        $userTbName = \Prj\Model\User::getTbname();
        $sql = "select * from ".$contractTbName." a where `status`=0 and EXISTS (select b.channelid from ".$userTbName." b where b.channelid=a.contractCode)";
        Loger::outVal("sql",$sql);
        $contractInfo = \Prj\Model\ContractInfo::query($sql);
        Loger::outVal("contractInfo",$contractInfo);
...
//                $contractId = substr_replace($v['contractCode'], $today, 4, 8);
                $regInfo = \Prj\Model\User::getRecords("oid", ['channelid' => $v['contractCode']]);
                $contractId = $v['contractCode'];
                if(empty($regInfo)) continue;
                $data['newRegNum'] = count($regInfo);  //注册人数
                $oidArr = array_column($regInfo, 'oid');
                $data['newBindNum'] = \Prj\Model\Payment\BankBind::getCount(['userId' => $oidArr, 'status' => "BIND", 'DATE_FORMAT(createTime,"%Y%m%d")' => $today]);    //新增绑卡人数
