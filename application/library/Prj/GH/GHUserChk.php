...
class GHUserChk {
    ...
    public function patch($phone)
    {
        if($this->phone==$phone){
            ...
            $uid = $this->db->getOne('jz_db.t_wfd_user', 'oid', array('userAcc'=>$phone));
        }
        $redis = $this->redis->getRecord('m', '*',array('investor'=>$uid));
        

        $mysql = $this->db->getRecord('jz_db.t_money_investor_baseaccount', '*',array('userOid'=>$uid));
...
//         }
    ...
    public function lock($phone)
    {
        ...
        $this->db->updRecords('jz_db.t_wfd_user', array('status'=>'forbidden'),array('oid'=>$uid));
        $this->db->updRecords('jz_db.t_money_investor_baseaccount', array('status'=>'forbidden'),array('userOid'=>$uid));
    ...
    public function unlock($phone)
    {
        ...
        $this->db->updRecords('jz_db.t_wfd_user', array('status'=>'normal'),array('oid'=>$uid));
        $this->db->updRecords('jz_db.t_money_investor_baseaccount', array('status'=>'normal'),array('userOid'=>$uid));
    ...
    {
        $memberid= $this->db->getOne('jz_db.t_account_order', 'userOid',array('orderNo'=>$orderNo));
        return $this->db->getOne('jz_db.t_wfd_user', 'userAcc', array('memberOid'=>$memberid));
    }
    public function findlockedAction()
    {
        $phones = $this->db->getCol('jz_db.t_wfd_user_bank', 'userOid');
        $lockedInUc = $this->db->getCol('jz_db.t_wfd_user', 'oid',array('oid'=>$phones,'!status'=>'normal'));
        $lockedInMimosa = $this->db->getCol('jz_db.t_money_investor_baseaccount', 'userOid',array('userOid'=>$phones,'!status'=>'normal'));
        ...
        $phones = $this->db->getPair('jz_db.t_wfd_user', 'userAcc','oid',array('oid'=>$phones));
    ...
    {
        $phones = $this->db->getCol('jz_db.t_wfd_user_bank', 'userOid');
        return $this->db->getCol('jz_db.t_wfd_user', 'userAcc',array('oid'=>$phones));
    }
    public function findActives($hour=100)//找出10天内有记录的用户（不论成功失败）
    {
        $dt = date('Y-m-d H:i:s', time()-3600*$hour);
        $us = array();
        $tmp = $this->db->getCol('jz_db.t_money_investor_tradeorder', 'distinct(investorOid)',array('>createTime'=>$dt));
        $us = $this->db->getCol('jz_db.t_money_investor_baseaccount', 'userOid',array('oid'=>$tmp));
        $tmp = $this->db->getCol('jz_db.t_money_investor_bankorder', 'distinct(investorOid)',array('>createTime'=>$dt));
        $us = array_merge($us ,  $this->db->getCol('jz_db.t_money_investor_baseaccount', 'userOid',array('oid'=>$tmp)));
        $tmp = $this->db->getCol('jz_db.t_money_publisher_investor_holdincome', 'distinct(investorOid)',array('>createTime'=>$dt));
        $us = array_merge($us ,  $this->db->getCol('jz_db.t_money_investor_baseaccount', 'userOid',array('oid'=>$tmp)));
        $us = $this->db->getCol('jz_db.t_wfd_user', 'userAcc',array('oid'=>array_unique($us)));
        return $us;
...
//         $members = $this->db->getCol('jz_db.t_bank_payment', 'userOid',
//             array('&'=>array('type'=>'02',
//                 '|1'=>array(
...
//                 )
//                 )));
//         $members = $this->db->getCol('jz_db.t_wfd_user', 'userAcc',array('memberOid'=>$members));
//         return $members;
    }
    public function username($userPhones)
    {
        $tmp = $this->db->getPair('jz_db.t_wfd_user', 'userAcc', 'oid',array('userAcc'=>$userPhones));
        $r = $this->db->getPair('jz_db.t_wfd_user_bank', 'userOid', 'name',array('userOid'=>$tmp));
    ...
    public function __construct()
    {
        ...
        $this->prdts = $this->db->getPair('jz_db.t_gam_product', 'oid', 'name');
        $this->currentProducts = $this->db->getCol('jz_db.t_gam_product','oid',array('type'=>'PRODUCTTYPE_02'));
        ...
        //$this->prdtsRepayed = $this->db->getCol('jz_db.t_gam_product', 'oid', array('repayInterestStatus'=>'repayed'));
        $this->prdtsRepayed = $this->db->getCol('jz_db.t_gam_product', 'oid', array('state'=>'CLEARED'));
    ...
    public $dbINI = array(
        'mysql'=>array('server'=>'127.0.0.1','port'=>3306,'dbType'=>'Myisam','pass'=>'c7La7bU6Q%k7','dbName'=>'jz_db','user'=>"root"),
    ...
    {
        $r = $this->db->getRecord('jz_db.t_wfd_user', '*',array('userAcc'=>$phone));
        if(empty($r)){
        ...
        $r = $this->db->getRecord('jz_db.t_wfd_user_bank', '*',array('userOid'=>$this->ucUID));
    ...
    {
        $r = $this->db->getRecord('jz_db.t_money_investor_baseaccount', '*',array('userOid'=>$this->ucUID));
        $this->lock['isFreshman'] = $r['isFreshman'];
        if($r['status']!=='normal'){
        ...
        $investorId = $r['oid'];
           ...
           //申购赎回等交易记录
        $rs = $this->db->getRecords('jz_db.t_money_investor_tradeorder', '*',array('investorOid'=>$investorId));
        foreach($rs as $r){
            //echo json_encode($r)."\n";
            $orderId = $r['orderCode'];
        ...
        if(!empty($this->prdtHold)){
            $rs = $this->db->getPair('jz_db.t_money_publisher_hold', 'productOid', 'totalInvestVolume',array('investorOid'=>$this->mimosaUID,'!accountType'=>'SPV'));
            foreach($rs as $id=>$n){
        ...
        foreach ($this->prdtHold as $id=>$r){
           ...
           //充值、提现、现金红包的交易记录
        $rs = $this->db->getRecords('jz_db.t_money_investor_bankorder', '*',array('investorOid'=>$investorId));
        foreach($rs as $r){
//            echo ">>".var_export($r,true);
    ...
    protected function infoInSettlement()
    {
        ...
        //         $investorId = $r['oid'];
        $r = $this->db->getPair('jz_db.t_account_info', 'accountType', 'balance', array('userOid'=>$this->memberId,'accountType'=>array('05','10')));
        ...
        $incomeFound=array();
        $rs = $this->db->getRecords('jz_db.t_account_order', '*',array('userOid'=>$this->memberId));
        foreach($rs as $r){
            ...
            //}else{
                $orderId = $r['orderNo'];
                if(strlen($orderId)>30 && ($type=='派息' || $type=='赎回')){//活期赎回
                    $orderId = $prdt.'@'.substr($r['createTime'],0,10);
                    $incomeFound[]=$prdt;
                }
            //}
...
//         $rs = $this->db->getRecords('jz_db.t_money_publisher_hold', 'productOid,totalInvestVolume,totalBaseIncome,updateTime',array('investorOid'=>$this->mimosaUID,'productOid'=>$incomeFound));
//         foreach ($rs as $r){
//             $prdt = $r['productOid'];
...
//         //根据产品，从mimosa那里查出派息赎回记录 -- 活期
        if(!empty($incomeFound)){
            $rs = $this->db->getRecords('jz_db.t_money_publisher_investor_holdincome', 'productOid,incomeAmount,updateTime',array('investorOid'=>$this->mimosaUID,'productOid'=>$incomeFound));
            foreach ($rs as $r){
                $prdt = $r['productOid'];
                $orderId = $prdt.'@'.substr($r['updateTime'],0,10);
...
//            error_log("----------- $orderId --------".\Sooh2\Util::to);
               //检查订单状态
    ...
    private function calcInOrder($orderId)
    {
        ...
        switch ($this->orders[$orderId]['type']){
            ...
            case '付息':
                ...
                break;
            case '普赎':
                error_log( "calc 普赎  + $orderAmount-$feeAmount $trace \n");
                $this->arrCalc['returnCurrent'] += $orderAmount-$feeAmount;
                if($prdt[0]=='#'){//活期
                    //echo " ？currentDeposit？ 赎回  - $orderAmount $trace \n";
                    $this->currentDeposit -=$orderAmount ;
                }
