 ...
 * @author lingtima@gmail.com
 */
class UserCheckin extends Base
{
...
//        $dbORM = $ModelUserFinal->dbWithTablename();
    ...
    {
//        \Sooh2\Misc\Loger::getInstance()->app_trace(json_encode($this->record));
        ...
        if (!$ModelUser->exists()) {
            if (M_RECORD_EXISTS) {
                $this->breakNums++;
                $this->breakData['not_exists'][] = $userId;
            }
            return false;
        } else {
            try {
                $checkinAmount = $ModelUser->getField('checkinAmount');
            } catch (\Exception $e) {
                $checkinAmount = 0;
            }
            try {
                $checkinNum = $ModelUser->getField('checkinNum');
            } catch (\Exception $e) {
                $checkinNum = 0;
            }
            if (empty($checkinAmount) && empty($checkinNum) && $userConponList = $this->getUserConpon($userId)) {
    ...
    {
        \Prj\Model\Flexible::reset(M_OLDDBCONF_NAME, 't_user_coupon');
