...
class NewbieReward extends \Rpt\Misc\ConfTpl\Main
{
    ...
    protected $configFlag = [
        'signin_register_bonus'     =>  '注册奖励金额',
        'signin_bindingCard_bonus'  =>  '绑卡奖励金额',
        'signin_investment_bonus'     =>  '投资奖励金额',
        'signin_recharge_bonus'            =>  '充值奖励金额'
    ];
    ...
    public function initForm($data){
        ...
        if(isset($data['id'])){
            ...
            foreach($configs as $v){
                $name = 'config#'.$v['flag'];
                if($v['flag'] == 'signin_amount_rand'){
                    $edtForm->addFormItem(\Prj\View\Bjui\TableForm::factory($name , $v['value'] , $this->configFlag[$v['flag']]));
                    continue;
                }
                $edtForm->addFormItem(\Sooh2\BJUI\FormItem\Text::factory($name, htmlspecialchars($v['value']), $this->configFlag[$v['flag']])->initChecker(new \Sooh2\Valid\Str(true, 1, 80)));
            }
        }else {
