...
class System extends \Rpt\Misc\ConfTpl\Main
{
    ...
    protected $configFlag = [
        'system_sms_switch' => '短信开关',
        'system_register_switch' => '注册开关',
        'system_login_switch' => '登录开关',
    ];
...
//
//                }
                $edtForm->addFormItem(\Sooh2\BJUI\FormItem\Text::factory($name, htmlspecialchars($v['value']), $this->configFlag[$v['flag']])->initChecker(new \Sooh2\Valid\Str(false, 0, 80)));
            }
        }else {
            foreach($this->configFlag as $k=>$v){
                $name = 'config#'.$k;

                $edtForm->addFormItem(\Sooh2\BJUI\FormItem\Text::factory($name,'',$v)->initChecker(new \Sooh2\Valid\Str(false,0,1000)));
            }
        }
