 ...
 * Time: 18:55
 */
class WebLabelController extends \Rpt\Manage\ManageIniCtrl
{
    public function indexAction()
    {
        ...
        $page = \Sooh2\BJUI\Pages\ListStd::getInstance()->init("Web文字替换")->initDatagrid($table)->initStdBtn(\Sooh2\Misc\Uri::getInstance()->uri(null, "pageAdd"));
    ...
    {
        $arr = \Prj\Model\DataTmp::getRecords("*", ['type' => 'web']);
        foreach ($arr as $k => $v) {
            $arr[$k]['value'] = htmlentities($v['value']);
    ...
    public function pageAddAction()
    {
        ...
        if ($form->isUserRequest($this->_request)) {
            ...
            $page = \Sooh2\BJUI\Pages\AddInDlg::getInstance()->init('Web文案替换')->initForm($form);
    ...
    public function pageupdAction()
    {
        ...
        if ($edtForm->isUserRequest($this->_request)) {
            ...
            $page = \Sooh2\BJUI\Pages\AddInDlg::getInstance()->init('Web')->initForm($edtForm);
            $this->renderPage($page);
        }
    }
